...
class Toast extends Component {
  ...
  render() {
    ...
    return (
      <View>
        <Animated.View  style={{ transform: [{ translateY: animation }], height: 70, 
         backgroundColor: this.state.toastColor, left:0, top: 0,
        ...
        <Animated.View style={{ transform: [{ translateX: this.animatedXValue }], height: 70,
         marginTop: windowHeight - 70, backgroundColor: this.state.toastColor, position: 'absolute', left:0,
          top:0, width: windowWidth, justifyContent: 'center' }}>
          <Text style={{ marginLeft: 10, color: 'white', fontSize:16, fontWeight: 'bold', 
           textAlign: 'center' }}>{ this.state.message }</Text>
        </Animated.View>
